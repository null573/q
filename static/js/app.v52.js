...
async function refreshCapacityData() {
    ...
    try {
        ...
        const data = await r.json();
        if (data.success) {
            const preloaded = data.before_refresh?.preloaded_models ?? 0;
            showToast(`产能刷新已启动（刷新前缓存 ${preloaded} 个型号），约10秒后完成`, 'success');
            // 3秒后自动重新计算当前表单的可发货日期（使用最新产能）
            setTimeout(() => {
                calculateDate();
            }, 3000);
            // 12秒后提示完成（并发5线程加载，约8-10秒完成）
            setTimeout(() => {
                showToast('产能数据刷新完成，已使用最新数据重新计算', 'success');
            }, 12000);
        } else {
            showToast('刷新启动失败：' + (data.error || '未知错误'), 'error');
        }
    } catch (e) {
